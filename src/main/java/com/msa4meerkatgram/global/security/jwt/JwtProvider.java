...
// @Component
// 클래스 레벨에서 빈으로 등록해서 관리해줘 빈 싱글톤으로 등록
// 하나의 객체만 생성해서 다른 곳에서 사용해줄 수 있음
@Component
public class JwtProvider {
    ...
    public JwtProvider(JwtConfig jwtConfig, CookieManager cookieManager){
        ...
        this.secretKey = Keys.hmacShaKeyFor(Decoders.BASE64.decode(jwtConfig.secret())); // 시크릿키 만들기
    ...
    private String generateToken(UserMybatis user, long ttl){
        ...
        return Jwts.builder()
                ...
                .and() // 추가 연결
                .subject(String.valueOf(user.getId())) // 유저를 특정하는 id셋팅에 주로 사용
