...
src/main/resources/
├── mapper/                              ← MyBatis SQL 매퍼 XML 파일 (auth, posts, user 등)
└── application.yaml                     ← 전체 애플리케이션 설정 파일
```
...
| **Service Layer** | 실제 애플리케이션의 **비즈니스 로직**을 실행하며, 트랜잭션(`@Transactional`)의 경계를 정의합니다. 필요 시 다수의 Mapper를 호출합니다. | Controller의 HTTP 세션이나 HttpServletRequest 등 웹 기술 관련 코드에 의존하지 않습니다. |
| **Mapper Layer (MyBatis)** | 데이터베이스 액세스 레이어로, **SQL 매핑 인터페이스**를 정의하고 대응되는 XML 파일의 SQL 쿼리를 실행하여 결과를 Java 객체로 매핑합니다. | 데이터베이스 쿼리 실행에만 충실하고 비즈니스 로직을 다루지 않습니다. |
